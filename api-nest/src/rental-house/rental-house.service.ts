...
import { Mansion, PrismaPromise, RentalHouse } from '@prisma/client';
...
export class RentalHouseService {
  ...
  findOne(id: string): PrismaPromise<RentalHouse> {
    return this.prismaService.rentalHouse.findUnique({
      where: { id },
      include: {
        rental_house_photos: {
  ...
  create({
    ...
    input: CreateRentalHouseInput;
    owner_id: string;
  }): PrismaPromise<RentalHouse> {
    return this.prismaService.rentalHouse.create({
      data: { ...input, owner_id },
    });
  }
  ...
  edit({ id, input }: { id: string; input: EditRentalHoseSystemInput }) {
